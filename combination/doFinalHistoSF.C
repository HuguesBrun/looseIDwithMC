...
doFinalHistoSF(){
    ...
    for (int k=0; k<5 ; k++){
        for (int j = 0 ; j<2 ; j++){
            for (int m = 0 ; m < 3 ; m++){
                TH2F *thePlotData = (TH2F*) theFile->Get(Form("effTrigger_ptbin%i_",j)+runPeriod[k]+"_"+theTypeName[m]+"_data");
                TH2F *thePlotMC = (TH2F*) theFile->Get(Form("effTrigger_ptbin%i_",j)+runPeriod[k]+"_"+theTypeName[m]+"_mc");
